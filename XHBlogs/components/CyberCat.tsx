...
export default function CyberCat() {
  ...
  // 🧠 记忆归档：把一轮对话存入短期记忆，超出 10 轮自动遗忘最旧的
  const remember = (userText: string, catReply: string) => {
    historyRef.current.push({ role: 'user', content: userText }, { role: 'assistant', content: catReply });
  ...
  const handleFeed = async (e: React.MouseEvent) => {
    ...
    try {
      const feedMessage = "我刚刚喂了你一条美味的小鱼干！你有什么表示？";
      const res = await fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: feedMessage, history: historyRef.current }),
      });

      if (!res.ok) throw new Error('API Error');

      const data = await res.json();
      remember(feedMessage, data.reply);
  ...
  const handleChatSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      remember(userMessage, data.reply);
      speak(data.reply, 8000);
    } catch (error) {
