...
export function MusicProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // 🌟 本地音乐库：直接来自 public/music/，不依赖云端；LRC 走下方现成的 lrcUrl 拉取逻辑
    const localSongs = ((siteConfig as any).localMusic || [])
      .filter((m: any) => m && m.src)
      .map((m: any, i: number) => ({
        id: `local-${i}`,
        title: m.title || '本地音乐',
        artist: m.artist || '未知歌手',
        cover: m.cover || '/images/music-cover.jpg',
        src: m.src,
        lrcUrl: m.lrc || null,
        lyrics: [],
      }));
    ...
    const fetchMusicData = async () => {
      try {
        ...
        // 🌟 本地歌曲在前，云端歌曲在后
        const mergedPlaylist = [...localSongs, ...cloudSongs];
        ...
        if (isMounted) {
          // 🌟 云端失败时，本地歌曲仍然可以正常播放
          if (localSongs.length > 0) setPlaylist(localSongs);
