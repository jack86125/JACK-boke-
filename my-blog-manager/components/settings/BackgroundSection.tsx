...
export default function BackgroundSection({ formData, handleUpdate, pushToQueue }: any) {
  ...
  // 【核心功能】：本地图片上传 (🌟 不再依赖图床,直接存进双端 public/images/)
  const handleFileUpload = async (file: File) => {
    ...
    setIsUploading(true);
    showToast("正在保存图片到本地图库...", "info");

    try {
      const configRes = await fetch(`/backend_config.json?t=${Date.now()}`);
      const configData = await configRes.json();

      // 构建 multipart/form-data (只需要图片文件本身)
      ...
      if (data.success && data.url) {
        showToast("🎉 图片已保存到本地！请确认是否加入背景库", "success");
  ...
  const cancelPendingImage = () => {
    setPendingImageUrl(null);
    showToast("已取消操作，但图片已保存在本地图库中", "info");
  };
          ...
          >
            ...
            <div className="text-center z-10">
              <p className="text-sm font-bold text-slate-700 dark:text-slate-200">
                {isUploading ? "正在保存到本地图库..." : "点击或将图片拖拽至此"}
              </p>
            </div>
        ...
        {pendingImageUrl && (
          ...
          >
            <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 max-w-sm w-full shadow-2xl border border-white/20">
              <h3 className="text-lg font-black text-slate-800 dark:text-white mb-4 text-center">✅ 图片保存成功！</h3>
