...
def get_site_config():
    ...
    try:
        ...
        # 1.5 🌟 隔离并解析 localMusic 数组（对象数组内含 title/artist/cover 等字段，
        # 不先剔除的话会被下面的通用正则当成站点外层配置抓走，造成数据串位）
        lm_match = re.search(r'localMusic\s*:\s*\[([\s\S]*?)\]', root_content)
        if lm_match:
            root_content = re.sub(r'localMusic\s*:\s*\[[\s\S]*?\]', '', root_content)
            parsed_config['localMusic'] = []
            for entry_match in re.finditer(r'\{([^{}]*?)\}', lm_match.group(1)):
                entry = {}
                # 🌟 key 兼容有无引号两种写法 (json.dumps 写入时带引号，手写时无引号)
                for m in re.finditer(r'"?([a-zA-Z0-9_]+)"?\s*:\s*"([\s\S]*?)"', entry_match.group(1)):
                    entry[m.group(1)] = m.group(2)
                if entry.get('src'):
                    parsed_config['localMusic'].append(entry)
...
def update_site_config(payload: Dict[str, Any] = Body(...)):
    ...
    VALID_ROOT_KEYS = {
        ...
        "bgImages", "defaultPostCover", "photoWallImage", "cloudMusicIds", "localMusic", "social",
