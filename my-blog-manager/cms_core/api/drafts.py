...
class StyledMarkdownConverter(MarkdownConverter):
    """span/mark 保留原始 HTML(含 style 属性)。
    编辑器(TipTap)的字体颜色/字号/荧光高亮就是这两个标签,
    markdownify 默认会把它们剥成纯文本导致样式丢失;
    博客端与编辑器端渲染管线均已开启 allowDangerousHtml,原样内嵌 HTML 可完整还原。"""
...
async def sync_local_operations(request: Request):
    ...
    for op in operations:
        if op.get("type") == "publish_article":
            ...
            # 2. 调用 markdownify 进行基础转换
            # 🌟 使用自定义转换器:span/mark 保留原始 HTML(字体颜色/字号/高亮不丢)
            md_content = StyledMarkdownConverter(heading_style="ATX").convert(raw_html)
