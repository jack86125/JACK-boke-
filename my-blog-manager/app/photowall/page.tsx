...
import { Plus, Pencil, Trash2, Search, Image as ImageIcon, X, Save, AlertTriangle, Sparkles, Edit3, CloudUpload, GripVertical } from 'lucide-react';
import { useOperations } from '../../context/OperationContext';
import { useToast } from '../../components/ToastProvider';
import FloatingImageTool from '../../components/editor/FloatingImageTool';

// 🌟 dnd-kit 拖拽手柄属性(useSortable 提供的激活器,贴到手柄按钮上)
...
// 🌟 相册卡片(拖拽手柄可选:传入 dragHandle 才渲染,搜索态不传)
function AlbumCard({ album, onOpen, onEdit, onDelete, dragHandle }: AlbumCardProps) {
  return (
    <div className="group cursor-pointer flex flex-col items-center relative">
      {dragHandle && (
        <div className={`absolute top-0 left-0 z-50 transition-all translate-x-4 group-hover:translate-x-0 ${dragHandle.isDragging ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
          <button
            ref={dragHandle.setActivatorNodeRef}
            {...dragHandle.attributes}
            {...dragHandle.listeners}
            title="拖拽调整相册顺序"
            className="p-2.5 bg-slate-600/80 backdrop-blur-md text-white rounded-xl shadow-lg hover:scale-110 transition-transform cursor-grab active:cursor-grabbing touch-none"
          >
      ...
      <div className="absolute top-0 right-0 z-50 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all -translate-x-4 group-hover:translate-x-0">
         <button onClick={(e) => { e.stopPropagation(); onEdit(); }} className="p-2.5 bg-indigo-500 text-white rounded-xl shadow-lg hover:scale-110 transition-transform"><Edit3 size={16}/></button>
         <button onClick={(e) => { e.stopPropagation(); onDelete(); }} className="p-2.5 bg-red-500 text-white rounded-xl shadow-lg hover:scale-110 transition-transform"><Trash2 size={16}/></button>
      </div>

      <div onClick={onOpen} className="relative w-[85%] aspect-[4/3] mb-8">
      ...
      <div className="text-center px-4 w-full" onClick={onOpen}>
...
// 🌟 可拖拽版本:dnd-kit 二维网格排序,手柄启动拖拽(整卡点击仍然进入相册)
function SortableAlbumCard(props: Omit<AlbumCardProps, 'dragHandle'>) {
  const { attributes, listeners, setNodeRef, setActivatorNodeRef, transform, transition, isDragging } = useSortable({ id: props.album.id });
  const style = {
  ...
  return (
    <div ref={setNodeRef} style={style} className={isDragging ? 'relative z-50' : 'relative'}>
      <AlbumCard {...props} dragHandle={{ setActivatorNodeRef, attributes, listeners, isDragging }} />
    </div>
  );
}
...
export default function PhotoWallPage() {
  ...
  const syncToQueue = (newAlbums: Album[], silent = false) => {
    addOperation({
    ...
    if (!silent) showToast("📍 变更已加入待处理队列，请在 Navbar 点击更新本地", "info");
  };

  // 🌟 拖拽排序:拖放结束才入暂存队列(同 type+label 自动去重,不刷屏)
  const sensors = useSensors(useSensor(PointerSensor, { activationConstraint: { distance: 5 } }));

  const handleDragEnd = (event: DragEndEvent) => {
    const { active, over } = event;
    if (!over || active.id === over.id) return;
    const oldIndex = editableAlbums.findIndex(a => a.id === active.id);
    const newIndex = editableAlbums.findIndex(a => a.id === over.id);
    if (oldIndex === -1 || newIndex === -1) return;
    const next = arrayMove(editableAlbums, oldIndex, newIndex);
    setEditableAlbums(next);
    syncToQueue(next, true);
  };
            ...
            <div className="animate-fade-in-up">
              ...
              <div className={`transition-opacity duration-300 ${isTransitioning ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-20 mt-10">
                  ...
                  {activeQuery ? (
                    // 搜索态:过滤后的子集不允许重排,渲染普通卡片(无拖拽手柄)
                    matchedAlbums.map((album) => (
                      <AlbumCard
                        key={album.id}
                        album={album}
                        onOpen={() => { setSearchQuery(''); setCurrentAlbum(album); }}
                        onEdit={() => setAlbumModal({ isOpen: true, mode: 'edit', data: album })}
                        onDelete={() => setDeleteModal({ isOpen: true, type: 'album', id: album.id, title: album.title })}
                      />
                    ))
                  ) : (
                    // 🌟 拖拽排序:dnd-kit 二维网格,顺序会写入 albums.ts 并同步到博客
                    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                      <SortableContext items={matchedAlbums.map(a => a.id)} strategy={rectSortingStrategy}>
                        {matchedAlbums.map((album) => (
                          <SortableAlbumCard
                            key={album.id}
                            album={album}
